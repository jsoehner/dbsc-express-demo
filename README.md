...
10. **Native DBSC vs polyfill `boundFetch`**: The `dbsc-toolkit` provides a `window.boundFetch` polyfill wrapper. If the browser successfully negotiated Native DBSC (which runs at the OS/browser network stack level), you should **not** use `boundFetch`. Using the polyfill wrapper while native DBSC is active will accidentally intercept the request and perform a Web Crypto signature, overwriting the session's native registration on the server and downgrading the tier from `dbsc` to `bound`. Always use a check (e.g., `isNative ? fetch : window.boundFetch`) before dispatching requests.
